# [#](https://funtl.com/zh/docs-docker/附录02：Dockerfile-最佳实践.html#dockerfile-最佳实践)Dockerfile 最佳实践

## [#](https://funtl.com/zh/docs-docker/附录02：Dockerfile-最佳实践.html#一般性的指南和建议)一般性的指南和建议

### [#](https://funtl.com/zh/docs-docker/附录02：Dockerfile-最佳实践.html#容器应该是短暂的)容器应该是短暂的

通过 `Dockerfile` 构建的镜像所启动的容器应该尽可能短暂（生命周期短）。「短暂」意味着可以停止和销毁容器，并且创建一个新容器并部署好所需的设置和配置工作量应该是极小的。

### [#](https://funtl.com/zh/docs-docker/附录02：Dockerfile-最佳实践.html#使用-dockerignore-文件)使用 `.dockerignore` 文件

使用 `Dockerfile` 构建镜像时最好是将 `Dockerfile` 放置在一个新建的空目录下。然后将构建镜像所需要的文件添加到该目录中。为了提高构建镜像的效率，你可以在目录下新建一个 `.dockerignore` 文件来指定要忽略的文件和目录。`.dockerignore` 文件的排除模式语法和 Git 的 `.gitignore` 文件相似。

### [#](https://funtl.com/zh/docs-docker/附录02：Dockerfile-最佳实践.html#使用多阶段构建)使用多阶段构建

在 `Docker 17.05` 以上版本中，你可以使用 `多阶段构建` 来减少所构建镜像的大小。

### [#](https://funtl.com/zh/docs-docker/附录02：Dockerfile-最佳实践.html#避免安装不必要的包)避免安装不必要的包

为了降低复杂性、减少依赖、减小文件大小、节约构建时间，你应该避免安装任何不必要的包。例如，不要在数据库镜像中包含一个文本编辑器。

### [#](https://funtl.com/zh/docs-docker/附录02：Dockerfile-最佳实践.html#一个容器只运行一个进程)一个容器只运行一个进程

应该保证在一个容器中只运行一个进程。将多个应用解耦到不同容器中，保证了容器的横向扩展和复用。例如 web 应用应该包含三个容器：web应用、数据库、缓存。

如果容器互相依赖，你可以使用 `Docker 自定义网络` 来把这些容器连接起来。

### [#](https://funtl.com/zh/docs-docker/附录02：Dockerfile-最佳实践.html#镜像层数尽可能少)镜像层数尽可能少

你需要在 `Dockerfile` 可读性（也包括长期的可维护性）和减少层数之间做一个平衡。

### [#](https://funtl.com/zh/docs-docker/附录02：Dockerfile-最佳实践.html#将多行参数排序)将多行参数排序
...
### [#](https://funtl.com/zh/docs-docker/附录02：Dockerfile-最佳实践.html#构建缓存)构建缓存
...
## [#](https://funtl.com/zh/docs-docker/附录02：Dockerfile-最佳实践.html#dockerfile-指令)Dockerfile 指令

下面针对 `Dockerfile` 中各种指令的最佳编写方式给出建议。

### [#](https://funtl.com/zh/docs-docker/附录02：Dockerfile-最佳实践.html#from)FROM

尽可能使用当前官方仓库作为你构建镜像的基础。推荐使用 [Alpine](https://hub.docker.com/_/alpine/) 镜像，因为它被严格控制并保持最小尺寸（目前小于 5 MB），但它仍然是一个完整的发行版。

### [#](https://funtl.com/zh/docs-docker/附录02：Dockerfile-最佳实践.html#label)LABEL
...
### [#](https://funtl.com/zh/docs-docker/附录02：Dockerfile-最佳实践.html#run)RUN

为了保持 `Dockerfile` 文件的可读性，可理解性，以及可维护性，建议将长的或复杂的 `RUN` 指令用反斜杠 `\` 分割成多行。

#### [#](https://funtl.com/zh/docs-docker/附录02：Dockerfile-最佳实践.html#apt-get)apt-get
...
### [#](https://funtl.com/zh/docs-docker/附录02：Dockerfile-最佳实践.html#cmd)CMD
...
### [#](https://funtl.com/zh/docs-docker/附录02：Dockerfile-最佳实践.html#expose)EXPOSE
...
### [#](https://funtl.com/zh/docs-docker/附录02：Dockerfile-最佳实践.html#env)ENV
...
### [#](https://funtl.com/zh/docs-docker/附录02：Dockerfile-最佳实践.html#add-和-copy)ADD 和 COPY
...
### [#](https://funtl.com/zh/docs-docker/附录02：Dockerfile-最佳实践.html#entrypoint)ENTRYPOINT
...
### [#](https://funtl.com/zh/docs-docker/附录02：Dockerfile-最佳实践.html#volume)VOLUME

`VOLUME` 指令用于暴露任何数据库存储文件，配置文件，或容器创建的文件和目录。强烈建议使用 `VOLUME` 来管理镜像中的可变部分和用户可以改变的部分。

### [#](https://funtl.com/zh/docs-docker/附录02：Dockerfile-最佳实践.html#user)USER
...
### [#](https://funtl.com/zh/docs-docker/附录02：Dockerfile-最佳实践.html#workdir)WORKDIR

为了清晰性和可靠性，你应该总是在 `WORKDIR` 中使用绝对路径。另外，你应该使用 `WORKDIR` 来替代类似于 `RUN cd ... && do-something` 的指令，后者难以阅读、排错和维护。

## [#](https://funtl.com/zh/docs-docker/附录02：Dockerfile-最佳实践.html#官方仓库示例)官方仓库示例

这些官方仓库的 Dockerfile 都是参考典范：https://github.com/docker-library/docs

上次更新: 2019-1-3 2:03:12

← [Docker 命令查询](https://funtl.com/zh/docs-docker/附录01：Docker-命令查询.html)[Docker 资源链接 ](https://funtl.com/zh/docs-docker/附录03：Docker-资源链接.html)→